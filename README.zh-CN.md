...
- 不把问题简化成“明天涨还是跌”，而是直接建模未来收益分布
- 将 `期末分布` 与 `路径触达概率` 明确拆成两个不同任务
- 使用 `GBM + 历史相似路径 bootstrap + LightGBM` 做概率融合
- 输出 CSV、Markdown、HTML 三种结果，适合程序消费和人工阅读
